...
uses
  Windows, WinSvc, SysUtils, Classes, Dialogs, PsAPI, Forms, Graphics;
...
begin
  ...
  begin
    try
      hService := 0;
      hService := CreateServiceW(hSCManager, 'ProcessMonitor', 'Process Monitor Driver', SERVICE_ALL_ACCESS, SERVICE_KERNEL_DRIVER, SERVICE_DEMAND_START, SERVICE_ERROR_NORMAL, PWideChar(DriverPath), nil, nil, nil, nil, nil);
      hService := 0;
      lpServiceArgVectors := nil;
      hService := OpenServiceW(hSCManager, 'ProcessMonitor', SERVICE_ALL_ACCESS);
      if hService <> 0 then
      begin
        try
  ...
  DriverDevice := CreateFileW('\\.\' + 'PROCESSMONITOR', GENERIC_READ or GENERIC_WRITE, 0, PSECURITY_DESCRIPTOR(nil), OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
end;
...
begin
  ...
  begin
    try
      hService := 0;
      hService := OpenService(hSCManager, 'ProcessMonitor', SERVICE_ALL_ACCESS);
      if (hService <> 0) then
      begin
        try
...
begin
  ...
  if InstallAndStartDriver(ExtractFilePath(ParamStr(0)) + 'ProcessMonitor.sys') then
